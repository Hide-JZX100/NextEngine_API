 ...
 *
 * 【ファイル構成と依存関係】
 * このプロジェクトは以下の5ファイルで構成されています。
 * 読み込み順序は GAS が自動管理しますが、依存関係は以下の通りです。
 *
 *   01_基盤構築.gs          ← このファイル (他ファイルの基盤、依存なし)
 *   02_店舗マスタ連携.gs    ← 01に依存
 *   03_ネクストエンジンAPI接続.gs ← 01に依存、NEAuthライブラリ必須
 *   04_データ整形・書き込み.gs   ← 01・02に依存
 *   05_メイン処理とエラーハンドリング.gs ← 全ファイルに依存
 *
 * ※ このファイル(01)は外部ライブラリへの依存はありません。
 * ※ NEAuthライブラリは 03 以降で使用します。
 *
 * 【テスト関数の推奨実行順序】
 * 初回セットアップ時は以下の順序で実行してください。
 *
 *   Step 1: testScriptConfig()     → スクリプトプロパティの設定確認
 *   Step 2: testDateCalculation1() → 日付計算ロジックの動作確認
 *   Step 3: testLogOutput()        → ログ出力レベルの動作確認
 *   Step 4: testPhase1()           → 上記3つの統合テスト
 *
 * ※ Step 1 でエラーが出た場合は、スクリプトプロパティを先に設定してください。
 * ※ 通常の動作確認は testPhase1() のみで問題ありません。
 *
 * 【注意事項・制約】
 * 1. GAS実行時間制限
 *    - 無料アカウント: 1回の実行につき最大6分
 *    - mainWithRetry と dailyUpdate は別トリガーで時間差実行すること
 *
 * 2. スクリプトプロパティ変更時の注意
 *    - ACCESS_TOKEN / REFRESH_TOKEN を手動変更した場合、
 *      他フェーズの処理にも即時反映されます
 *    - LOG_LEVEL を 1(全件出力) にすると実行ログが大量になります。
 *      本番運用時は 3(出力なし) を推奨します
 *
 * 3. タイムゾーン
 *    - GASプロジェクトのタイムゾーンを Asia/Tokyo に設定してください
 *    - 設定場所: GASエディタ「プロジェクトの設定」→「タイムゾーン」
 */
