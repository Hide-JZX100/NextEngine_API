 ...
 * キー                     | 値
 * -------------------------|------------------------------------
 * TRIGGER_FUNCTION_NAME2    | 実行したい関数名（例: updateStock）
...
// スクリプトプロパティのキー定義
const PROPERTY_KEY_FUNCTION2 = 'TRIGGER_FUNCTION_NAME2';
...
function setTrigger2() {
    ...
    const functionToTrigger = properties.getProperty(PROPERTY_KEY_FUNCTION2);
    const triggerMode = properties.getProperty(PROPERTY_KEY_MODE);

    // 必須プロパティのチェック
    if (!functionToTrigger) {
        Logger.log(`エラー: スクリプトプロパティ '${PROPERTY_KEY_FUNCTION2}' が設定されていません。`);
        return;
    }
