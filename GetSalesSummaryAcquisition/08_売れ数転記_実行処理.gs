 ...
 * 
 * スクリプトプロパティ DEBUG_TIMING を 'true' に設定すると、
...
function copyMakerData(config) {
  try {
    // コピー元を開く
    const debugTiming = PropertiesService.getScriptProperties()
      .getProperty('DEBUG_TIMING') === 'true';
    const t0 = Date.now();
    ...
    if (debugTiming) {
      console.log(`[TIMING] ${config.name}: 元を開く=${t1-t0}ms / 先を開く=${t2-t1}ms / 開始行計算=${t3-t2}ms / 元読込=${t4-t3}ms / 先書込=${t5-t4}ms / 合計=${t5-t0}ms`);
    }
 ...
 */
function sendErrorNotification1(subject, body) {
  try {
...
URL: https://script.google.com/home/projects/${ScriptApp.getScriptId()}/edit
    ...
    sendErrorNotification1(subject, body);
