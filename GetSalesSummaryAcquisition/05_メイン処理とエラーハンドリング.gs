 ...
 *
 * 【ファイル構成と依存関係】
 * このファイルは全フェーズのファイルに依存しています。
 *
 * 01_基盤構築.gs への依存:
 *   - getScriptConfig()    : 設定値・リトライ回数・通知先メールの取得
 *   - getSearchDateRange() : 検索期間(7日前〜本日)の計算
 *   - getLogLevelName()    : ログレベル名の取得
 *   - logMessage()         : ログ出力制御
 *
 * 02_店舗マスタ連携.gs への依存:
 *   - getShopMapWithCache() : キャッシュ付き店舗マスタ取得
 *
 * 03_ネクストエンジンAPI接続.gs への依存:
 *   - searchReceiveOrderRowAll() : 受注明細全件取得(ページネーション対応)
 *
 * 04_データ整形・書き込み.gs への依存:
 *   - convertAllToSpreadsheetData() : APIデータのスプレッドシート形式への変換
 *   - writeToSpreadsheet()          : スプレッドシートへの書き込み
 *
 * ※ Phase 1〜4 がすべて正しく動作することを先に確認してください。
 * ※ NEAuthライブラリ(識別子: NEAuth)がバージョン5で追加されている必要があります。
 *
 * 【テスト関数の推奨実行順序】
 * 初回セットアップ時は以下の順序で実行してください。
 *
 *   Step 1: testEmailNotification()  → メール通知の送信確認 ※実際にメールが届きます
 *   Step 2: testMain()               → メイン処理の動作確認 ※APIとスプレッドシートが更新されます
 *   Step 3: testMainWithRetry()      → リトライ機能の動作確認 ※Step 2と同様
 *   Step 4: testPhase5()             → Step 1の統合テスト
 *
 * ※ トリガーに設定する関数は main() ではなく mainWithRetry() を推奨します。
 *   理由: 一時的なAPIエラー発生時に自動リトライするためです。
 * ※ integratedDailyUpdate() は内部に5分待機が含まれます。
 *   GASの6分実行時間制限を超える可能性があるため、単独での使用は推奨しません。
 * ※ 通常の動作確認は testMain() のみで問題ありません。
 *
 * 【注意事項・制約】
 * 1. トリガー設定時の推奨関数
 *    - トリガーには mainWithRetry() を設定してください
 *    - main() を直接設定した場合、一時的なAPIエラーで処理が中断されます
 *    - リトライ回数は RETRY_COUNT スクリプトプロパティで変更可能です(省略時3回)
 *
 * 2. リトライ間隔の仕様
 *    - リトライ間隔は試行回数に応じて増加します
 *      1回目失敗 → 5秒待機 → 2回目実行
 *      2回目失敗 → 10秒待機 → 3回目実行
 *      3回目失敗 → 全リトライ失敗としてエラー通知メールを送信
 *
 * 3. メール送信の上限
 *    - GASのメール送信には1日あたり100通の上限があります
 *    - NOTIFY_ON_SUCCESS を true にすると1日3回の成功通知が送信されます
 *    - エラーが頻発した場合はリトライ分も加算されるため注意してください
 *    - 本番運用時は NOTIFY_ON_SUCCESS を false(省略時のデフォルト)を推奨します
 *
 * 4. integratedDailyUpdate() の使用上の注意
 *    - 内部に5分(300秒)の待機処理が含まれています
 *    - GASの実行時間制限(6分)を超える可能性があるため
 *      mainWithRetry() と dailyUpdate() は別トリガーで時間差実行してください
 */
