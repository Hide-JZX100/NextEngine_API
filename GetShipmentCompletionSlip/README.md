...
| ファイル名 | 役割 |
|-----------|------|
| `00_NE_認証ライブラリ使用必須関数.gs` | 認証関連の補助関数 |
| `01_config.プロジェクト設定.gs` | 全体設定（API URL、フィールド定義、バッチ設定） |
| `02_utils.ユーティリティ.gs` | 日付計算・ウォームアップ処理 |
| `03_api.ネクストエンジンAPI.gs` | ネクストエンジンAPI呼び出し・トークン管理 |
| `04_process.フィルタとマッピング.gs` | データフィルタリング・加工処理 |
| `05_sheet.スプレッドシート書込.gs` | Sheets APIによるスプレッドシート書き込み |
| `06_main.メイン処理.gs` | メイン処理・バッチ実行・自己スケジューリング |
| `07_cleanup.クリーンアップ処理.gs` | 古いデータ・トリガーのクリーンアップ |
