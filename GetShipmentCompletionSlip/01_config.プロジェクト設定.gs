/**
 * プロジェクト設定ファイル (GetShipmentCompletionSlip)
 * 
 * 【目的】
 * このフォルダ（GetShipmentCompletionSlip）に含まれるGASプロジェクト全体で使用する
 * 設定値、定数、および環境依存のパラメータを一元管理するために作成されました。
 * コード内にハードコードされるべきでない値をこのファイルに集約することで、
 * 保守性と可読性を向上させています。
 * 
 * 【含まれる主な設定項目】
 * - 開発/本番モードの切り替え設定
 * - ネクストエンジンAPIのエンドポイントと通信設定
 * - 出力先GoogleスプレッドシートのIDとシート名
 * - 取得データのフィールド定義とヘッダーマッピング
 * 
 * 【含まれる関数】
 * - isDevelopmentMode(): スクリプトプロパティを参照して現在の動作モードを判定するヘルパー関数
 */
...
const CONFIG = {
    ...
    // バッチ処理設定
    BATCH: {
        // 各バッチの取得日付範囲（前月の何日～何日）
        RANGES: [
            { start: 1, end: 10 },   // バッチ1: 1日～10日
            { start: 11, end: 20 },  // バッチ2: 11日～20日
            { start: 21, end: 31 }   // バッチ3: 21日～末日（実際の末日で自動調整）
        ],
        INTERVAL_MINUTES: 5        // バッチ間のインターバル（分）
    },
