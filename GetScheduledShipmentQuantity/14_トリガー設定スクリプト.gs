...
function setTrigger() {
    ...
    // 既存のトリガーを削除（メイン関数 + ウォームアップ関数）
    deleteTriggersForFunction(functionToTrigger);
    deleteTriggersForFunction('warmUpNextEngineConnection');

    // 実行したい時刻と関数名の配列
    // functionName を省略した場合は functionToTrigger（メイン関数）が使われる
    const executionTimes = [
        { hour: 7, minute: 35, functionName: 'warmUpNextEngineConnection' },
        { hour: 7, minute: 56 },    // 7:56
        { hour: 8, minute: 57 },    // 8:57
        { hour: 9, minute: 57 },    // 9:57
        { hour: 12, minute: 27 },   // 12:27
        { hour: 13, minute: 27 },   // 13:27
    ];
    ...
    executionTimes.forEach(function (time) {
        ...
        const funcName = time.functionName || functionToTrigger;
        ...
        try {
            ...
            Logger.log(`  ✓ 作成: ${dateStr} ${timeStr} [${funcName}]`);
            createdCount++;
