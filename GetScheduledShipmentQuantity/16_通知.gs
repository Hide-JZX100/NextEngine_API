 ...
 * ============================================================================
 * 【概要】
 * ネクストエンジンAPI連携処理におけるエラー発生時のメール通知用共通モジュール。
 * スクリプト所有者（Session.getEffectiveUser()）宛てに、発生した障害の重要度に応じた
 * メール通知を送信します。
 *
 * 【関数一覧】
 * 1. notifyByEmail(severity, subject, body)
 *    - エラー発生時にメールを送信する共通関数。呼び出し元の処理を遮断しない安全設計。
 * 2. testNotifyByEmail()
 *    - notifyByEmail() の動作確認用テスト関数（手動実行用）。
 *
 * 【作成日】 2026-07-22
 * ============================================================================
 */
