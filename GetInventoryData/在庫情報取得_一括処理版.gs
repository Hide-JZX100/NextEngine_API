...
function setLogLevelDebug() {
  const scriptProperties = PropertiesService.getScriptProperties();
  scriptProperties.setProperty('LOG_LEVEL', '3');
  console.log('ログレベルをDEBUGに変更しました（詳細ログモード）');
}

function setLogLevelInfo() {
  const scriptProperties = PropertiesService.getScriptProperties();
  scriptProperties.setProperty('LOG_LEVEL', '2');
  console.log('ログレベルをINFOに変更しました（通常モード）');
}

function setLogLevelError() {
  const scriptProperties = PropertiesService.getScriptProperties();
  scriptProperties.setProperty('LOG_LEVEL', '1');
  console.log('ログレベルをERRORに変更しました（エラーのみモード）');
}
...
function checkCurrentLogLevel() {
  ...
  console.log(`現在のログレベル: ${levelNames[level] || level}`);
  return level;
}
