...
*/
function testStockMasterFields(goodsCode = "0001-12104-d-zo") {
  try {
...
function testSingleUpdate() {
  ...
  updateSingleProduct("0001-12104-d-zo");
}



/**
 * ネクストエンジン在庫情報取得プロジェクト
 * 認証ライブラリ(NEAuth v2.0)を使用
 */
...
function testInventoryApi() {
  console.log('=== 在庫APIテスト ===');
  
  // 自分のプロジェクトのスクリプトプロパティを取得
  const myProperties = PropertiesService.getScriptProperties();
  
  // ライブラリのテスト関数を呼び出し
  const result = NEAuth.testInventoryApi(myProperties);
  
  console.log('在庫取得テスト成功!');
  console.log('商品情報:', result);
...
function clearMyTokens() {
  ...
  console.log('トークンをクリアしました。再認証が必要です。');
}
...
function runFullTest() {
  ...
  try {
    // ステップ1: トークン情報確認
    console.log('【ステップ1】トークン情報確認');
    showMyTokens();
    console.log('');
    
    // ステップ2: API接続テスト
    console.log('【ステップ2】API接続テスト');
    testApiConnection();
    console.log('');
    
    // ステップ3: 在庫APIテスト
    console.log('【ステップ3】在庫APIテスト');
    testInventoryApi();
    console.log('');
    ...
    console.error('');
    console.error('トークンが未設定の場合は、以下を実行してください:');
    console.error('1. testGenerateAuthUrl() で認証URLを取得');
    console.error('2. ブラウザで認証を完了');
    console.error('3. 再度このテストを実行');
  }
}
 ...
 * 実際の在庫情報取得処理（これから実装）
 */
function getInventoryToSheet() {
  console.log('=== 在庫情報取得開始 ===');
  
  // このプロジェクトのプロパティを取得
  const myProperties = PropertiesService.getScriptProperties();
  const accessToken = myProperties.getProperty('ACCESS_TOKEN');
  const refreshToken = myProperties.getProperty('REFRESH_TOKEN');
  
  if (!accessToken || !refreshToken) {
    console.error('認証が必要です。testGenerateAuthUrl()を実行してください。');
    return;
  }
  
  // 在庫取得のロジックをここに実装
  console.log('在庫取得処理を実装予定...');
  
  // 例: ネクストエンジンAPIを直接呼び出し
