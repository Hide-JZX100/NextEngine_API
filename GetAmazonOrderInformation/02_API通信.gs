/**
 * @file 02_API通信.gs
 * @description ネクストエンジン API通信基盤モジュール
 *
 * 【概要】
 * ネクストエンジンAPIとの低レイヤな通信、および受注データのページネーション取得を制御します。
 * 
 * 【主な機能】
 * - callNeApi: 共通のPOSTリクエスト実行、アクセストークンの自動更新（ローテーション）、
 *   および指数バックオフによるリトライ制御。
 * - fetchOrdersByShipDate: 特定の出荷日の受注データを全件取得するページネーション処理。
 *
 * 【エラーハンドリング】
 * - API側で 'token' を含むエラーメッセージが返った場合は、リトライせず即座に例外をスローします。
 * - ネットワーク一時不通などのエラー時は、定数(API_RETRY_MAX)に基づき指数バックオフでリトライします。
 *
 * 【依存関係】
 * - 01_設定ファイル.gs (各種定数、店舗ID取得関数)
 * - Google Apps Script (UrlFetchApp, PropertiesService)
 * 
 * 【認証仕様】
 * - リクエストごとに最新の access_token, refresh_token を使用します。
 * - レスポンスに含まれる新しいトークンは自動的にスクリプトプロパティへ保存されます。
 * - 実行前に「00_NE_認証ライブラリ」等で初期トークンが設定されている必要があります。
 */

/**
 * ネクストエンジンAPIへの単一リクエストを実行する共通関数です。
 * 
 * この関数は、APIリクエストの送信、アクセストークンの自動更新（ローテーション）、
 * および一時的な通信エラーに対する指数バックオフを用いたリトライ制御を一元管理します。
 *
 * 【処理フロー】
...
/**
 * 指定された出荷確定日に基づいて、出荷確定済みの受注データを全件取得します。
 * 
 * ネクストエンジンの受注検索APIは一度に取得できる件数に上限（最大10,000件）があるため、
 * 全データを取得するために `offset` と `limit` を用いたページネーション処理を行います。
 *
 * 【処理フロー】
 * 1. offset = 0 で開始
...
/**
 * 【開発・検証用】Phase 2：API通信および受注データ取得の動作確認テスト
 * 
 * 開発環境（テスト店舗等）の設定を使用して、実際にAPIからデータが取得できるかを確認します。
 *
 * 【処理フロー】
...
/**
 * 【開発・検証用】Phase 5-2：エラーリトライロジックの動作確認テスト
 * 
 * 意図的に存在しないAPIエンドポイントへリクエストを送信し、
 * `callNeApi` 関数内の指数バックオフによるリトライ処理が正しく実行されるかを確認します。
 */
