/**
 * @file 01_設定ファイル.gs
 * @description ネクストエンジン Amazon受注データ取得 - 定数・設定定義
 *
 * @summary
 * ネクストエンジンAPIを利用して受注データを取得・出力するための共通設定、定数、
 * およびスクリプトプロパティへのアクセッサを管理します。
 * 
 * @description
 * ## 主な機能
 * - APIエンドポイントおよび通信制御（リトライ・ウェイト）の定数定義
 * - 取得対象フィールドとスプレッドシート用ヘッダーのマッピング管理
 * - 開発モード(isDevelopmentMode)による接続先店舗の切り替え制御
 * - スクリプトプロパティ経由での動的設定（シートID等）の読み込み
 *
 * ## 依存関係
 * - Google Apps Script (PropertiesService)
 * - ネクストエンジン API v1 (受注伝票検索)
 * 
 * ## 必須/推奨スクリプトプロパティ
 * - OUTPUT_SPREADSHEET_ID: 出力先スプレッドシートのID
 * - OUTPUT_SHEET_NAME: 出力先のシート名（未設定時は 'API'）
 * - DEVELOPMENT_MODE: 'true' の場合に開発用店舗(楽天市場店)のIDを使用
 */
